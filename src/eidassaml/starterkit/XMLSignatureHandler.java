 ...
 * be used for validating the signatures. </p>
 * <p>
...
public class XMLSignatureHandler
{
   ...
   */
  public static void addSignature(SignableXMLObject signable,
                                  PrivateKey key,
                                  X509Certificate cert,
                                  SigEntryType type,
                                  String sigDigestAlg) throws CertificateEncodingException
  {
	//TODO rewirte this methode! because  this methode will use very time   SHA256-PSS
    ...
    if ("EC".equalsIgnoreCase(keyAlg) || "ECDSA".equalsIgnoreCase(keyAlg))
    {
      ...
      else if ("SHA256".equals(sigDigestAlg) || "SHA-256".equals(sigDigestAlg) || "SHA256-PSS".equals(sigDigestAlg)) //SHA256-PSS at the moment sigDigestAlg will be always SHA256-PSS 
      {
    ...
    else if ("RSA".equalsIgnoreCase(keyAlg))
    {
      ...
      {
        sig.setSignatureAlgorithm(org.apache.xml.security.signature.XMLSignature.ALGO_ID_SIGNATURE_RSA_SHA256_MGF1);
      }
      else if ("SHA384".equals(sigDigestAlg) || "SHA-384".equals(sigDigestAlg))
      {
    ...
    {
      ((SAMLObjectContentReference)sig.getContentReferences().get(0)).setDigestAlgorithm(EncryptionConstants.ALGO_ID_DIGEST_SHA256);
    }
  }
   ...
   * @param key signature key
   * @param cert certificate matching the signature key
   * @param sigDigestAlg a algo from the org.apache.xml.security.signature.XMLSignature algo catalog
   * @throws CertificateEncodingException
   */
  public static void addSignature(SignableXMLObject signable,
          PrivateKey key,
          ...
          String sigDigestAlg) throws CertificateEncodingException
  {
	  ...
	  Signature sig = new SignatureBuilder().buildObject();
	    ...
	    sig.setSignatureAlgorithm(sigDigestAlg);
	    sig.setCanonicalizationAlgorithm(org.apache.xml.security.encryption.XMLCipher.EXCL_XML_N14C);
	    KeyInfo keyInfo = new KeyInfoBuilder().buildObject();
	    X509Data x509Data = new X509DataBuilder().buildObject();
	    addCertificate(cert, x509Data);
	    keyInfo.getX509Datas().add(x509Data);
	    ...
	    if(org.apache.xml.security.signature.XMLSignature.ALGO_ID_SIGNATURE_RSA_SHA256_MGF1.equals(sigDigestAlg)){
	    	((SAMLObjectContentReference)sig.getContentReferences().get(0)).setDigestAlgorithm(org.apache.xml.security.encryption.XMLCipher.SHA256);
	    }else if(org.apache.xml.security.signature.XMLSignature.ALGO_ID_SIGNATURE_RSA_SHA512_MGF1.equals(sigDigestAlg)){
	    	((SAMLObjectContentReference)sig.getContentReferences().get(0)).setDigestAlgorithm(org.apache.xml.security.encryption.XMLCipher.SHA512);
	    }
  ...
  {
    org.opensaml.xml.signature.X509Certificate xmlcert = new X509CertificateBuilder().buildObject();
   ...
   */
  public static int checkSignature(Signature sig, X509Certificate... trustedAnchor) throws ErrorCodeException
  {
